defmodule LoomkinWeb.UserAuth do
  use LoomkinWeb, :verified_routes
  ...
  @remember_me_cookie "_loomkin_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      LoomkinWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  In local mode (multi_tenant: false), this passes through — no auth required.
  In deployed mode (multi_tenant: true), this enforces authentication.
  """
  def require_auth_if_multi_tenant(conn, _opts) do
