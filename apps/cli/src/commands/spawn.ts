...
const BUILT_IN_ROLES = ["researcher", "coder", "reviewer", "tester", "lead", "concierge"];
...
register({
  ...
  handler: async (args: string, ctx: CommandContext) => {
    ...
    ctx.addSystemMessage(
      pc.dim(
        `Spawning ${role} agent${name ? ` "${name}"` : ""}${worktree ? " (with worktree)" : ""}…`,
      ),
    );

    // Load agent-scoped memories to include as additional system prompt
    const agentMemories = [...loadAgentMemories(role), ...(name ? loadAgentMemories(name) : [])];
    const agentMemoryPrompt =
      agentMemories.length > 0 ? formatMemoriesForPrompt(agentMemories) : null;
    ...
    channel
      ...
      .receive("error", (resp: Record<string, unknown>) => {
        ...
        ctx.addSystemMessage(pc.red(`Failed to spawn agent: ${reason}`));
      });
  },
});
